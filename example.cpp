...
void readBallFile(pair<int, int> ballOutput) {
  ifstream file("OutputBalls.csv");
  string line;

  if (file.is_open()) {
    int i = 0;
    while (getline(file, line) && i < sizeof(ballOutput)/sizeof(*ballOutput)) {
      vector<string> v = split(line, ',');
      ballOutput[i] = make_pair(atoi(v[0].c_str()), atoi(v[1].c_str()));
      i++;
    }
  }
}

void placeBalls(TableState ts, pair<int, int> balls) {
...
void tryAllShots(TableState ts, ) {
...
int main(int argc, char * const argv[]) {
  ...
  readBallFile(balls); // balls is the output array
